...
if __name__ == "__main__":
    ...
    files = glob.glob('*.gxml')
    edict = {'Network':[],'PFL':[], 'Type1':[], 'Type2':[],
    'MISA':[], 'MISSA':[], 'uMISSA':[]}
    for file in files:
        graph = nx.convert_node_labels_to_integers(nx.read_graphml(file))
        edict["Network"].append(file.replace(".gxml", ""))
        ...
        edict["PFL"].append(result[0])
        edict["Type1"].append(result[1])
        edict["Type2"].append(result[2])
        edict["MISA"].append(result[3])
        edict["MISSA"].append(result[4])
        edict["uMISSA"].append(result[5])
    df = pd.DataFrame(edict)
    df.to_csv(args.net + "_hiLoops.csv")
    # print('PFL', result[0], '\nType1', result[1], '\nType2', result[2], '\nMISA', result[3], '\nMISSA', result[4], '\nuMISSA', result[5], sep='\t')
    if args.checknfl:
